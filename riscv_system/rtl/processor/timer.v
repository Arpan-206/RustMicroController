...
reg  [31:0] count;
reg  [31:0] limit;
reg         terminal;                      /* Status set when 'counting' to 0 */
reg         overrun;                   /* Terminal reached > once without ack */                         
reg   [3:1] control;
reg         enabled;
...
assign modulo = control[1];
assign once   = control[2];
...
always @ (posedge clk)
if (reset) overrun <= 1'b0;
else if (enabled && clk_en && match && terminal) overrun <= 1'b1;
     else if (clear_term) overrun <= 1'b0;

assign ctrl_rd = {terminal, overrun, 25'h0, terminal, control, enabled};
...
assign ireq_o = terminal && control[3];           /* Terminal count and IrqEn */
