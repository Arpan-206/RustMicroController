...
impl Rect {
    ...
    fn draw(self, colour: Colour) {
        draw_rect(self.x0, self.y0, self.x1, self.y1, colour);
    }
}
...
pub fn run() -> ! {
    ...
    while i < generated_scene::OBJECT_COUNT {
        ...
        previous[i] = rect_at(object.keyframes, 0);
        previous[i].draw(Colour(object.colour));
        i += 1;
    }
    ...
    loop {
        ...
        i = 0;
        while i < generated_scene::OBJECT_COUNT {
            next[i] = rect_at(generated_scene::OBJECTS[i].keyframes, frame);
            i += 1;
        }

        i = 0;
        while i < generated_scene::OBJECT_COUNT {
            previous[i].draw(background);
            i += 1;
        }

        i = 0;
        while i < generated_scene::OBJECT_COUNT {
            next[i].draw(Colour(generated_scene::OBJECTS[i].colour));
            previous[i] = next[i];
            i += 1;
        }
...
fn rect_at(keyframes: &[Kf], frame: u16) -> Rect {
    if keyframes.is_empty() {
        return Rect::empty();
    }

    if frame <= keyframes[0].frame {
        return rect_from_kf(&keyframes[0]);
    }
    ...
    rect_from_kf(&keyframes[keyframes.len() - 1])
}

fn rect_from_kf(keyframe: &Kf) -> Rect {
    Rect {
        x0: keyframe.x0,
        y0: keyframe.y0,
        x1: keyframe.x1,
